...
var LamPrintPdfTools = (function () {
  ...
  var init = function init() {
    ...
    $("#print-pdf-tools__scale").keyup(function (event) {
    ...
    $("#print-pdf-tools__orientation").change(function () {
      showPrintArea();
    });

    $("#print-pdf-tools__paper").change(function () {
      showPrintArea();
    });

    //adding tool reset
    LamToolbar.addResetToolsEvent({ eventName: "clear-layer-print-pdf" });

    //events binding
    LamDispatcher.bind("show-print-pdf-tools", function (payload) {
      LamToolbar.toggleToolbarItem("print-pdf-tools");
      if (LamToolbar.getCurrentToolbarItem() === "print-pdf-tools") {
        LamPrintPdfTools.showPrintArea();
        LamStore.setInfoClickEnabled(false);
      }
      lamDispatch("clear-layer-info");
    });

    LamDispatcher.bind("print-map-pdf", function (payload) {
      LamPrintPdfTools.printMapPdf(payload.paper, payload.orientation, payload.format, payload.template);
    });

    LamDispatcher.bind("clear-layer-print-pdf", function (payload) {
      LamMap.clearLayerPrint();
    });

    LamDispatcher.bind("show-print-pdf-area", function (payload) {
      ...
      if (scale) {
        ...
        LamPrintPdfTools.setScale(LamMap.getMapScale() / 2);
      }
      var paper = payload.paper;
      var orientation = payload.orientation;
      var printSize = LamPrintPdfTools.getPrintMapSize(paper, orientation, resolution);
  ...
  var templatePrint = function () {
    ...
    template += '<button id="print-tools__print-button" onclick="LamPrintPdfTools.printClick(); return false;" class="lam-btn" >Stampa</button>';
   ...
   */
  let printMapPdf = function (paper, orientation, format, template) {
    ...
    var paperDimension = LamPrintPdfTools.getPaperSize(paper, orientation);
    ...
    var scale = LamPrintPdfTools.getScale() * LamMap.aspectRatio(centerLL[1] * 1.12, centerLL[1] * 0.88);
