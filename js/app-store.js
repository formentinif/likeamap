...
var AppStore = (function() {
  ...
  /**
   * Mostra il pannelo del menu
   */
  var panelContentItemSelected = "";

  var init = function() {
    //ricavo la configurazione iniziale
    //var url = "/www/js/config.js";
    // $.ajax({
    //   dataType: "json",
    //   url: url,
    // }).done(function (data){
  ...
  /**
   * Resetta tutti i controllo al cambio di menu
  ...
  var showMenuContent = function(content, keepOpen) {
    ...
    if (panelContentItemSelected != content) {
      ...
      // $(".al-menu-panel-content-item").animate({
      //     width: 'hide'
      // }, 0);
    ...
    //eseguo delle azioni personalizzate
    //resetto funzioni generali
  ...
  var showInfoItem = function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      //if (data.length > 0) {
      //body += "<div class='al-h6'>Risultato " + (i + 1)  + "</div>"
      //}
   ...
   * @return {null}            La funzione non restituisce un valore ma una promise asincrona
   */
  ...
  */
  var mapInit = function(state, callback) {
    ...
    SearchTools.render("search-tools", appState.searchProvider, appState.searchProviderAddressUrl, appState.searchProviderAddressField, appState.searchProviderHouseNumberUrl, appState.searchProviderHouseNumberField, getSearchLayers());
